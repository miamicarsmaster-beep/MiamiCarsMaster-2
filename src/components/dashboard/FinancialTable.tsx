            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 px-2">
                ...
                <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
                    ...
                    <DialogContent className="sm:max-w-[600px] p-0 overflow-hidden border-0 bg-transparent shadow-none">
                        <div className="bg-card border border-border flex flex-col max-h-[90vh] rounded-[2.5rem] shadow-2xl">
                            <div className="relative px-8 py-10 overflow-hidden flex-shrink-0 text-center">
                                <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-primary to-transparent opacity-50" />
                                <DialogHeader>
                                    <DialogTitle className="text-3xl font-black italic tracking-tighter uppercase leading-none mb-3 text-foreground">
                            ...
                            <div className="px-10 pb-10 space-y-8 overflow-y-auto flex-1 custom-scrollbar">
                                <div className="space-y-3">
                                    ...
                                    <Select value={formData.vehicle_id} onValueChange={(value) => setFormData({ ...formData, vehicle_id: value })}>
                                        <SelectTrigger className="h-12 bg-secondary/50 border-border rounded-xl font-bold italic text-sm">
                                            <SelectValue placeholder="Seleccionar vehículo para cargo" />
                                        </SelectTrigger>
                                        <SelectContent className="z-[600] bg-popover border-border shadow-2xl">
                                            {vehicles.map((v) => (
                                ...
                                <div className="grid grid-cols-2 gap-6">
                                    <div className="space-y-3">
                                        ...
                                        <Select value={formData.type} onValueChange={(value: FinancialRecord["type"]) => setFormData({ ...formData, type: value })}>
                                            <SelectTrigger className="h-12 bg-secondary/50 border-border rounded-xl font-black italic uppercase text-sm">
                                                <SelectValue />
                                            </SelectTrigger>
                                            <SelectContent className="z-[600] bg-popover border-border shadow-2xl">
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <div className="relative group">
                                            ...
                                            <Input
                                                ...
                                                className="h-12 pl-10 bg-secondary/50 border-border rounded-xl font-black italic text-xl tracking-tighter"
                                                placeholder="0.00"
                                            />
                                        </div>
                                    </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                    <div className="space-y-3">
                                        ...
                                        <Input
                                            ...
                                            className="h-12 bg-secondary/50 border-border rounded-xl font-bold italic text-sm"
                            ...
                            <DialogFooter className="px-10 py-8 border-t border-border flex items-center justify-end gap-4 bg-secondary/20">
                                <Button
                                    variant="ghost"
                                    onClick={() => setIsAddOpen(false)}
                                    className="h-12 px-8 rounded-xl font-black uppercase tracking-widest text-[11px] hover:bg-muted transition-all"
                                >
                                    Abortar
                                </Button>
                                <Button
                        ...
                        <Table>
                            ...
                            <TableBody>
                                {records.length === 0 ? (
                                    ...
                                    records.map((record) => (
                                        <TableRow key={record.id} className="group hover:bg-primary/5 border-border transition-all">
                                            ...
                                            <TableCell className={`text-right font-black italic text-lg tracking-tighter ${record.type === 'income' ? 'text-emerald-500 dark:text-emerald-400' : 'text-red-500 dark:text-red-400'}`}>
                    ...
                    <div className="bg-card border border-border flex flex-col max-h-[90vh] rounded-[2.5rem] shadow-2xl">
                        {selectedRecord && (
                            <>
                                <div className="relative px-6 md:px-8 py-8 md:py-10 overflow-hidden flex-shrink-0">
                                    ...
                                    <div className="flex flex-col md:flex-row justify-between items-start gap-4">
                                        <div>
                                            ...
                                            <DialogTitle className="text-2xl md:text-4xl font-black italic tracking-tighter uppercase leading-none text-foreground mb-2">
                                                {selectedRecord.type === 'income' ? 'Ingreso' : 'Gasto'} <span className="text-primary">Operativo</span>
                                            </DialogTitle>
                                            <p className="text-[11px] font-black text-primary/60 uppercase tracking-[0.3em]">
                                                Hash ID: {selectedRecord.id.slice(0, 16)}...
                                            </p>
                                        </div>
                                        <div className={`text-right font-black italic text-3xl md:text-4xl tracking-tighter ${selectedRecord.type === 'income' ? 'text-emerald-500 dark:text-emerald-400' : 'text-red-500 dark:text-red-400'}`}>
                                ...
                                <DialogFooter className="px-10 py-8 border-t border-border flex items-center justify-center bg-secondary/10">
                                    <Button
                                        variant="ghost"
